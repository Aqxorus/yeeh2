...
createCommand({
  ...
  async execute(interaction, options) {
    ...
    const perms = new Permissions(
      await calculateMemberPermissions(guild, interaction.member)
    );
    ...
    if (!kickMembersPerm) {
      await interaction.respond(
        "You don't have the necessary permissions to warn a members (this command requires `Kick members`)"
      );
      return;
    }
    ...
    try {
      ...
      await interaction.respond(
        `Could not warn user <@${user.user.id}> | They likely do not have their DMs open.`
      );
      return;
    }
